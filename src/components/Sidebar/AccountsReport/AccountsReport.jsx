...
const AccountsReport = () => {
  ...
  return (
    <>
      <div>
        <li className='mb-1'>
          <span className={`btn btn-info d-inline-flex align-items-center rounded border-info border-2 account-report-header w-100 ${activeMenu === 'account-report-collapse' ? '' : 'collapsed'}`}
            onClick={() => handleMenuClick('account-report-collapse')}
            aria-expanded={activeMenu === 'account-report-collapse'}
          >
            <FaUniversalAccess className='me-2' /> Account Report
          </span>
          <div className={`collapse ps-2 ${activeMenu === 'account-report-collapse' ? 'show' : ''}`} id="account-report-collapse">
